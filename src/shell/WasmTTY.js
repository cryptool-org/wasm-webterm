...
class WasmTTY {
  ...
  constructor() {
    this.#promptPrefix = "" // current prompt prefix
    this.#continuationPromptPrefix = "" // current continuation prompt prefix

    this.#cursor = 0 // track current cursor position (for insert / delete)
    this.#inputBuffer = "" // track current user input
  }

  activate(xterm) {
    this.#xterm = xterm // keep reference to the underlying terminal

    this.#termSize = {
      cols: this.#xterm.cols, // get current number of columns fitting the screen
      rows: this.#xterm.rows, // get current number of rows fitting the screen
    }

    // register resize handler
    this.#xterm.onResize((data) => this.handleTermResize(data))
  }
  ...
  _clearInput() {
    ...
    if (moveRows > 0) this.#xterm.write("\x1b[E".repeat(moveRows)) // move line down

    // clear all input line(s)
    this.#xterm.write("\r\x1b[K") // clear line
    for (let i = 1; i < numRows; ++i) this.#xterm.write("\r\x1b[F\x1b[K") // move line up and clear line
  }

  /** Clear the entire terminal and move cursor to the top-left corner */
  clearTty() {
    this.#xterm.write("\x1b[2J") // Clear the whole screen
    this.#xterm.write("\x1b[0;0H") // move cursor to 0,0
  ...
  moveCursorTo(newCursor) {
    ...
    if (rowDiff > 0) {
      this.#xterm.write("\x1b[B".repeat(rowDiff)) // move down
    } else if (rowDiff < 0) {
      this.#xterm.write("\x1b[A".repeat(-rowDiff)) // move up
    }

    // adjust for horizontal difference
    const colDiff = newCol - currentCol
    if (colDiff > 0) {
      this.#xterm.write("\x1b[C".repeat(colDiff)) // move right
    } else if (colDiff < 0) {
      this.#xterm.write("\x1b[D".repeat(-colDiff)) // move left
    }
  ...
  _setInput(newInput) {
    ...
    if (this.#cursor > newInput.length) this.#cursor = newInput.length
    ...
    this.#xterm.write("\r") // move to column 0
    if (moveUpRows > 0) this.#xterm.write("\x1b[F".repeat(moveUpRows)) // move line up
    if (col > 0) this.#xterm.write("\x1b[C".repeat(col)) // move right
